...
#include "find_blank_bufferpool.hpp.hpp"
...
namespace sjtu {
    ...
    private:
        ...
        void save_info() {
            ...
            buffer_write(&rear, 0, sizeof(off_t), filename);
            buffer_write(&root, 0, sizeof(off_t), filename);
        }
        ...
        inline void buffer_load_node(_buffer b, const node &x) const{
            ...
            buffer_read(b, x.pos+sizeof(node), x.curSize*(sizeof(off_t)+sizeof(Key)), filename);
        }

        inline void buffer_load_leaf(_buffer b, const node &x) const{
            if(x.curSize == 0) return;
            buffer_read(b, x.pos+sizeof(node), x.curSize*(sizeof(value_t)+sizeof(Key)), filename);
        }

        inline void buffer_save_node(_buffer b, const node &x){
            buffer_write(b, x.pos+sizeof(node), x.curSize*(sizeof(off_t)+sizeof(Key)), filename);
        }

        inline void buffer_save_leaf(_buffer b, const node &x){
            buffer_write(b, x.pos+sizeof(node), x.curSize*(sizeof(value_t)+sizeof(Key)), filename);
        }
    ...
    public:
                ...
                node_size((K - sizeof(node))/(sizeof(Key)+sizeof(off_t))-1)
        {
            ...
            if(!file) {
                ...
                buffer_read(&rear, 0, sizeof(off_t), filename);
                buffer_read(&root, 0, sizeof(off_t), filename);
            }
        }
